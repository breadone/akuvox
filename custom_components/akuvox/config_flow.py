...
class AkuvoxFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_email_password_sign_in(self, user_input=None):
        """Step 1: User signs in with email and password."""
        data_schema = self.get_email_password_sign_in_schema(user_input)
        ...
        if user_input is not None:
            ...
            subdomain = (
                ...
                else helpers.get_subdomain_from_country_code(
                    helpers.get_country_phone_code_from_name(
                        user_input.get("country_code", "")
                    )
                )
            )
            ...
            if email and password:
                login_successful = (
                    await self.akuvox_api_client.async_email_password_login(
                        hass=self.hass,
                        email=email,
                        password=password,
                        subdomain=subdomain,
                ...
                if login_successful:
                    ...
                    return self.async_show_form(
                        step_id="email_password_sign_in",
    ...
    async def async_step_sms_sign_in(self, user_input=None):
        ...
        if user_input is not None:
            country_code = helpers.get_country_phone_code_from_name(
                user_input.get("country_code")
            )
            phone_number = (
                user_input.get("phone_number", "").replace("-", "").replace(" ", "")
            )
            subdomain: str = user_input.get("subdomain", "Default")
            subdomain = (
                subdomain
                if subdomain != "Default"
                else helpers.get_subdomain_from_country_code(country_code)
            )

            location_dict = helpers.get_location_dict(country_code)
            LOGGER.debug(
                "User will use the API subdomain '%s' for %s",
                subdomain,
            ...
            return self.async_show_form(
                ...
                description_placeholders=user_input,
                last_step=False,
                errors={"base": "Please enter a valid country code and phone number."},
            )
    ...
    async def async_step_app_tokens_sign_in(self, user_input=None):
        ...
        if user_input is not None:
            country_code: str = helpers.get_country_phone_code_from_name(
                user_input.get("country_code")
            )  # type: ignore
            phone_number: str = (
                user_input.get("phone_number", "").replace("-", "").replace(" ", "")
            ...
            if all(
                len(value) > 0
                for value in (country_code, phone_number, token, auth_token)
            ):
                ...
                if login_successful is True:
                    ...
                    ################################
                    return self.async_create_entry(
                        title=self.akuvox_api_client.get_title(), data=self.data
                    )
                else:
            ...
            return self.async_show_form(
                ...
                description_placeholders=user_input,
                last_step=True,
                errors={"base": "Please check the values enterted and try again."},
            )
    ...
    async def async_step_verify_sms_code(self, user_input=None):
        ...
        data_schema = {
            vol.Required(
                "sms_code",
                msg=None,
                description="Enter the code from the SMS you received on your device.",
            ): str,
        }
        ...
        if user_input is not None and user_input:
            ...
            return self.async_show_form(
                ...
                description_placeholders=user_input,
                last_step=True,
                errors={"sms_code": "Invalid SMS code. Please enter the correct code."},
            )
    ...
    def get_email_password_sign_in_schema(self, user_input: dict = {}):
        """Get the schema for the email/password sign-in step."""
        user_input = user_input or {}

        default_country_name_code = helpers.find_country_name_code(
            str(COUNTRY_PHONE.get(self.hass.config.country, ""))
        )
        default_country_name = LOCATIONS_DICT.get(default_country_name_code, {}).get(
            "country"
        )
        ...
        return {
            vol.Required(
                ...
                description="Your SmartPlus account email address",
            ): str,
            vol.Required(
                "password",
                description="Your SmartPlus account password",
            ): str,
            vol.Required(
                "country_code",
                default=default_country_name,
                description="Your country (used to select the regional API server)",
            ...
            vol.Optional(
                "subdomain",
                default="Default",
                description="Manually override the regional API subdomain",
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
    ...
    def get_app_tokens_sign_in_schema(self, user_input: dict = {}):
        ...
        return {
            ...
            vol.Required(
                ...
                description="Your SmartPlus account's auth_token string",
            ): str,
            vol.Required(
                "token",
                msg=None,
                default=user_input.get("token", DEFAULT_TOKEN),  # type: ignore
                description="Your SmartPlus account's token string",
            ): str,
            vol.Optional(
                "subdomain",
                default="Default",  # type: ignore
...
class AkuvoxOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        default_country_name_code = helpers.find_country_name_code(
            config_data.get("country_code", self.hass.config.country)
        )
        default_country_name = LOCATIONS_DICT.get(default_country_name_code, {}).get(
            "country"
        )  # type: ignore
        default_subdomain = LOCATIONS_DICT.get(default_country_name_code, {}).get(
            "subdomain"
        )  # type: ignore
        ...
        wait_for_image_url = (
            True
            if user_input.get("event_screenshot_options", "asap") == "wait"
            else False
        )
        ...
        if self.akuvox_api_client is None:
            ...
            self.akuvox_api_client._data.auth_token = self.get_data_key_value(
                "auth_token"
            )  # type: ignore
            self.akuvox_api_client._data.token = self.get_data_key_value("token")  # type: ignore
            self.akuvox_api_client._data.phone_number = self.get_data_key_value(
                "phone_number"
            )  # type: ignore
            self.akuvox_api_client._data.wait_for_image_url = self.get_data_key_value(
                "wait_for_image_url"
            )  # type: ignore
        ...
        if user_input.get("override", False) is True:
            ...
            if await self.akuvox_api_client.async_init_api() is True:
                ...
                await self.akuvox_api_client.async_retrieve_user_data_with_tokens(
                    user_input["auth_token"], user_input["token"]
                )
                devices_json = self.akuvox_api_client.get_devices_json()
                if devices_json is not None and all(
                    key in devices_json
                    for key in ("camera_data", "door_relay_data", "door_keys_data")
                ):
                    ...
                    options_schema = vol.Schema(
                        {
                            vol.Required(
                                "token", default=config_options.get("token", None)
                            ): str,
                            vol.Optional("camera_data", default=camera_data): dict,
                            vol.Optional(
                                "door_relay_data", default=door_relay_data
                            ): dict,
                            vol.Optional(
                                "door_keys_data", default=door_keys_data
                            ): dict,
                        }
                    )
                else:
